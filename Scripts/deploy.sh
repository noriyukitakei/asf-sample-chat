...
appplicationCount=`sfctl application list | grep ChatApplication | wc -l`
...
if [[ "$appplicationCount" -eq "0" ]];then
    ...
    applicationUpgradeCheck= `sfctl application manifest --application-type-name ChatApplicationType --application-type-version $applicationTypeVersion &>/dev/null`
    if [[ "$?" -eq  "0" ]]; then
